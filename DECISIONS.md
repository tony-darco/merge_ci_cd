...
### 34. Triage escalates on uncorroborated *terminal refusals*, not only on schema-validation failure

**Decision:** `run_triage` re-checks with `TRIAGE_FALLBACK_MODEL` whenever the primary model returns a category outside `{CODE_DEFECT, DEPENDENCY_ISSUE, FLAKY}` that no deterministic detector independently confirmed — in addition to the original escalate-on-`SchemaValidationFailure` path from decision #16.
**Context:** Decision #16 anticipated that `qwen3.5:0.8b` would struggle with a six-way classification and built a fallback ladder for it, but wired that ladder to `SchemaValidationFailure` — malformed JSON. Live M5 measurement showed the real failure mode is different and more common: the small model returns *well-formed but semantically wrong* answers, so the ladder never fires. Measured on the `code-defect` fixture: `qwen3.5:0.8b` classified correctly in 8/12 runs (67%), the other 4 as `FLAKY`, plus an observed live `CONFIG_OR_SECRET`; `qwen3.5:latest` scored 8/8 on identical input (LOG.md).
**Alternatives considered:** (a) Promote `qwen3.5:latest` to the primary triage model — fixes accuracy but pays the larger model's cost on every single run including the common, easy path, and abandons decision #16's tiering rationale entirely. (b) Escalate on *every* classification — same cost problem, strictly worse. (c) Leave it and document the accuracy figure — rejected: a 1-in-3 chance of silently refusing a genuinely fixable failure is a product defect, not a footnote. (d) Majority vote across N samples of the small model — more calls than a single escalation and still no better than the small model's ceiling on inputs it systematically misreads.
**Consequences:** The extra call is paid only when the run would otherwise terminate, so the common path keeps the cheap model. Correctness is asymmetric by design and this makes that explicit: a wrong `FLAKY` costs one retry and self-corrects (the retry reproduces the real bug and the graph continues to Diagnosis), whereas a wrong `INFRA_FAILURE`/`CONFIG_OR_SECRET`/`UNKNOWN` ends the run with no recourse. Note the deterministic detectors (#17, #22) still guard the *dangerous* direction — never "confidently fixing" a real OOMKill or auth failure — since they run before the LLM and short-circuit it; this decision guards the opposite direction, refusing work that should have been done.
**Confidence:** High on the mechanism and on the measured numbers. Medium on the 67% figure generalizing — it is one fixture, and a broader accuracy sweep across all seeds is an M7 item.

### 35. The sandbox verifies its own isolation at startup and fails closed

**Decision:** `agentic-fixer-verify-wft` runs an `await-network-policy` init container that probes a deliberately non-allowlisted target (`1.1.1.1:443`, by raw IP) and refuses to start the test container while that probe still succeeds. If egress is still open after 60s, the workflow fails rather than running the code anyway.
**Context:** M5 established (LOG.md) that kube-router populates a NetworkPolicy's source ipset only *after* it observes a pod, leaving a startup window in which a pod in the deny-by-default namespace egresses freely. A `pytest` run is short enough to complete inside that window, so the isolation the sandbox exists to provide could be absent for exactly the workload it is meant to contain — model-authored code.
**Alternatives considered:** (a) Document the race and accept it — rejected; the sandbox's entire justification is that untrusted code cannot reach the network, and a guarantee that holds "usually" is not one. (b) A fixed `sleep` before the workload — sound but arbitrary: it trades a real check for a guess, and the correct duration is unknowable in general. (c) Query the CNI or node state to confirm the ipset is populated — needs node-level access the sandbox deliberately does not grant, and couples the manifest to kube-router specifically. (d) Give the pod a long-lived sidecar so it outlives the sync — mitigates the race without ever demonstrating enforcement.
**Consequences:** Isolation is now proven per-run rather than assumed, and the failure mode is refusal rather than silent exposure. Probing by raw IP matters: DNS *is* allowlisted, so a name-resolution failure would prove nothing about egress. One residual ambiguity is acknowledged in the code: a probe that is blocked on the very first attempt cannot, from inside the pod, distinguish "already enforced" from "target unreachable for unrelated reasons". That was checked empirically rather than assumed — `1.1.1.1:443` is reachable from an unpoliced namespace in this cluster, and blocked from the first probe inside the sandbox, so enforcement is in fact already active by the time an init container starts. The security-relevant direction is sound regardless: the gate only ever *fails closed*, since the branch it cannot disambiguate is the one where egress is already denied.
**Confidence:** High on the mechanism and the fail-closed property. Medium on portability — the timing characteristic is kube-router's, and a different CNI would need its own empirical check.

### 36. Mandatory reasoning is enforced at the schema boundary, not only at the trace boundary

**Decision:** `reasoning` (and `root_cause`/`rationale`) are constrained in `agent/llm/schema.py` to be non-empty after whitespace stripping, in addition to `agent/trace.py`'s validator that raises on empty reasoning.
**Context:** The trace validator is a hard requirement of this project — a decision with no reasoning is a bug, not a logging gap. But it fires at `record_trace` time, which is *after* the decision has been made and acted upon. Observed live: `qwen3.5:0.8b` intermittently returns an empty reasoning string, which crashed the run at the trace boundary rather than anywhere useful.
**Alternatives considered:** (a) Catch the trace `ValidationError` and substitute a placeholder — rejected outright; it converts the guarantee into a lie, which is precisely what the validator exists to prevent. (b) Leave it as a crash — the guarantee holds, but a recoverable model glitch takes down a whole run for no reason. (c) Retry the LLM call manually at each call site — duplicates logic that `generate_structured` already implements correctly.
**Consequences:** An empty-reasoning response now fails schema validation, which `generate_structured` already handles by re-prompting with the validation error appended, so the model is told it must explain itself and asked again. Same guarantee, recoverable failure mode. The trace validator stays exactly as it is, as the last line of defense — it should simply be rare for it to be the one that fires.
**Confidence:** High.
